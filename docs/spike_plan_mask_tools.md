...
- Installing `torch` or other heavy ML frameworks
- Installing SAM / SAM2 or downloading model checkpoints
- Installing Stable Diffusion / `diffusers` or similar large model stacks
- Installing LaMa, heavy inpainting stacks, or geospatial libraries (rasterio / geopandas)
- Copying entire external repositories into the VikVec core
- Changing VikVec core around one external repo before a spike succeeds
...
- `LabelMe` or `segment-anything-annotator` — the top priority is review and polygon correction UX. These spikes are low-risk ways to validate whether mask-based review workflows reduce artifact leakage compared to bbox-first crops.
